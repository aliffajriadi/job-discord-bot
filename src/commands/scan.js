...
export default {
  name: "scan",
  description: "Scan world dengan Sorting (Terbanyak di atas) & Easy Maintenance",
  ...
  async execute(message, args) {
    ...
    try {
      ...
      const allDropped = (result.data.dropped || []).sort((a, b) => b.count - a.count);
      const allTiles = (result.data.tiles || []).sort((a, b) => b.count - a.count);
      ...
      const generateEmbed = (page) => {
        ...
        const droppedList = currentDropped
          .map((d) => `${getEmoji(d.name)} **${d.name}**: \`${d.count.toLocaleString()}\``)
          .join("\n") || "✨ Tidak ada FLOATING.";

        const tilesList = currentTiles
        ...
        return new EmbedBuilder()
          ...
          .setThumbnail("https://ik.imagekit.io/8zzj11dsp/Growtopia/GrowScan_9000.webp")
          .setDescription(`Berhasil menscan world **${result.world}**`)
          .addFields(
            { name: "<a:1000768724548206622:1482437232961781954> Scanner Bot", value: `\`${result.bot}\``, inline: true },
            { name: "<a:1438378765335138386:1482437228176216227> Status", value: "`Success`", inline: true },
            { name: "━━━━━━━━━━━━━━━━━━━━━━", value: "\u200B" },
            { name: "**BLOCKS**\n\n", value: tilesList },
            { name: "\u200B", value: "\u200B" },
            { name: "**FLOATING**\n\n", value: droppedList }
      ...
      collector.on("collect", async (interaction) => {
        if (interaction.user.id !== message.author.id) {
          return interaction.reply({ content: "Kamu tidak punya akses ke tombol ini!", ephemeral: true });
        }
